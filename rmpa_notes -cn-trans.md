...
```
    AAAA AAAA 0000 0000 BBBB BBBB CCCC CCCC  
    DDDD DDDD EEEE EEEE FFFF FFFF GGGG GGGG  
    HHHH HHHH IIII IIII 0000 0000 0000 0000  
```
**RMPA Header**, which is about 0x30 in length:  
+ `A` 0x00 RMP file type string, or PMR because of byte-swapping.
    > 文件头 决定大小端
+ `B` 0x08 boolean denoting if routes are to be processed.
    > 路线?  
    > 是否唯一值, 下一块重复不  
+ `C` 0x0C offset to route data.
    > 默认0x30  
+ `D` 0x10 boolean denoting if shapes are to be processed.
    > 形状?  
    > 是否唯一值, 下一块重复不  
+ `E` 0x14 offset to shape data.
+ `F` 0x18 boolean denoting if camera paths are to be processed.
    > 相机?  
    > 是否唯一值, 下一块重复不   
+ `G` 0x1C offset to camera path data.
+ `H` 0x20 boolean denoting if spawnpoints are to be processed.
    > 刷新点
    > 就目前解析来看, 肯定是 True
+ `I` 0x24 offset to spawnpoint data.
    > 刷新点数据块的偏移地址
 When there are less types of data (only spawns for example) the unused offsets will be duplicate pointers.  
    > 当数据类型较少时（例如仅生成），未使用的偏移量将是重复的指针。
...
```
    ...
    0000 0000 CCCC CCCC DDDD DDDD EEEE EEEE 
```
...
```
    ...
    CCCC CCCC 0000 0000 0000 0000 DDDD DDDD  0x1F  
...
+  `A` 0x08 is the string offset naming the shape type (rectangle, sphere, whatwasthethird).
    > 是命名形状类型（矩形，球形，whatthetheth）的字符串偏移量。
